...
public class PositionCreateDbValidatorTests
{
    private static PositionCreateDbValidator GetValidator(
        IMock<IPositionRepository> repository)
    {
        var builder = new ContainerBuilder();
        builder.RegisterInstance(repository.Object);
        builder.RegisterType<PositionCreateDbValidator>();
        builder.Populate([]);
        var container = builder.Build();
        return container.Resolve<PositionCreateDbValidator>();
    }

    [Fact]
    public async Task Position_Positive_Create_New_Record()
    {
    ...
    [Fact]
    public async Task Position_Negative_Create_New_Record_Has_Duplicate()
    {
        var position = PositionData.PositionModel();
